...
mlr<-readRDS("mlrmodel_coef4.rds")
```
...
plot(calishapefile,col='darkorchid', lwd = .1,add=TRUE)
plot(coldshapefile, col='skyblue', lwd = .1,add=TRUE)
plot(nsshapefile,col='darkolivegreen3', lwd = .1,add=TRUE)
plot(sgsshapefile,col='gold', lwd = .1,add=TRUE)
plot(hotshapefile,col='indianred', lwd = .1,add=TRUE)

legend('bottomleft', legend=c("California annuals","Northern mixed prairies",
                              "Cold Deserts ","Shortgrass steppe", "Hot deserts"),
       col=c("darkorchid", "darkolivegreen3","skyblue","gold","indianred"), lty=1,
       bty='n',lwd=2,cex=1.60,box.lty=0, )
...
grid1 <- grobTree(
  rasterGrob(image1),
  textGrob(label1, x = 0.05, y = 0.8, hjust = 0, vjust = 1, gp = gpar(fontsize = 14)))

grid2 <- grobTree(
  rasterGrob(image2),
  textGrob(label2, x = 0.05, y = 0.8, hjust = 0, vjust = 1, gp = gpar(fontsize = 14)))
