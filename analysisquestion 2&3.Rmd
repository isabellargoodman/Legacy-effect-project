...
lm_map_coef<-readRDS("lm_map_coef2.rds")
lm_herb_coef<-readRDS("lm_herb_coef2.rds")
lm_currentPPT_coef<-readRDS("lm_currentPPT_coef2.rds")
```
...
mapInteraction<-ggplot()+
  geom_density(data=map_coefficients, aes(x=Spatiotemporal, fill=ecoregion,after_stat(scaled)),alpha=.4)+ 
...
geom_vline(xintercept = 0, size =.75, linetype = "dashed", color = "black")+
 theme_classic() + 
  labs(fill = "Ecoregion")+
...
herbInteraction<-ggplot()+
  geom_density(data=herb_coefficients, aes(x=Spatiotemporal.herb, fill=ecoregion,after_stat(scaled)),alpha=.4)+ 
  scale_fill_manual(values=c(northern='lightblue',cali='grey',cold='gold',hot='firebrick3',sgs='green4'),
                    labels=c(northern='Northern mixed prairies',
                             cali='California annuals',cold='Cold deserts',hot='Hot deserts',sgs='Shortgrass steppe'))+
 xlab("Change in legacy effect \n per % herbaceous NPP")+
...
currentInteraction<-ggplot()+
  geom_density(data=current_coefficients, aes(x=Spatiotemporal.current, fill=ecoregion,after_stat(scaled)),alpha=.4)+
...
legeffect_byEco<-ggplot()+
  geom_density(data=current_coefficients, aes(x=Temporal.current, fill=ecoregion, after_stat(scaled)), alpha=.4)+
  scale_fill_manual(values=c("firebrick3","gold","grey","lightblue","green4"),
                       labels=c("Hot deserts","Cold deserts","California annuals", "Northern mixed prairies","Shortgrass steppe"))+
 ...
 theme_classic() + 
  labs(fill = "Ecoregion")+
  theme(legend.key.size = unit(.65, 'cm'), #change legend key size
...
# Convert 'PPTdev' and 'lagNPPDev' to factors
legacydata 
legacydata$PPTdev <- as.factor(legacydata$PPTdev)
legacydata$lagNPPdev <- as.factor(legacydata$lagNPPdev)
...
interaction.plot(
  x.factor = df$PPTdev,
  trace.factor = df$lagNPPDev,
  response = df$NPPdev,
  type = "b",  # "b" for points connected by lines
  legend = TRUE,
  col = c("blue", "red"),  # customize colors if needed
  pch = c(16, 17)  # customize symbols if needed
)

# Add labels and title
title("Interaction Plot: PPTdev vs. NPPdev by lagNPPDev")
xlabel("PPTdev")
ylabel("NPPdev")
...
total_rows <- nrow(calileggy)

sample_size <- round(0.0001 * total_rows)

# Randomly select 10% of the dataset
random_sample <- calileggy[sample(1:total_rows, size = sample_size), ]

random <- legacydata[sample(nrow(legacydata), 1), ]


print(random_observation)

calicurrent<-current_coefficients%>%
  filter(ecoregion=="cali")

calileggy<-legacydata%>%
  filter(ecoregion=="cali")

randomcali<-calileggy%>%
  filter(ID==308569)

ggplot(randomcali,aes(x=PPT,y=NPP))+
  geom_point()+
...
```{r}
random_sample$PPTdev <- as.factor(random_sample$PPTdev)   
random_sample$lagNPPdev <- as.factor(random_sample$lagNPPdev)

interaction.plot(
  x.factor = random_sample$PPTdev,
  trace.factor = random_sample$lagNPPdev,
  response = random_sample$NPPdev,
  type = "b",  # "b" for points connected by lines
  legend = TRUE,
  col = c("blue", "red"),  # customize colors if needed
  pch = c(16, 17)  # customize symbols if needed
...
lm<-lm(NPPdev~PPTdev+lagNPPdev+PPTdev:lagNPPdev, data=calileggy) 
summary(lm)
```
...
ggplot(random_sample, aes(x = PPTdev, y = NPPdev, color = lagNPPdev)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE) +
  labs(title = "Interaction Plot", x = "PPTdev", y = "NPPdev", color = "lagNPPdev")+
