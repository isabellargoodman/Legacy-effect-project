...
MAP<-legacydata%>%
  ...
  summarise_at(vars(PPT),
               list(MAP = mean))
legacydata<-left_join(x=legacydata, y=MAP, by=c("x", "y", "ecoregion")) 
...
```{r}
write_rds(lm_map_coef, "lm_map_coef.rds")
write_rds(lm_currentPPT_coef, "lm_currentPPT_coef.rds")
write_rds(lm_herb_coef, "lm_herb_coef.rds")

write_rds(lm_map_aic, "lm_herb_aic.rds")
write_rds(lm_currentPPT_aic, "lm_currentPPT_aic.rds")
write_rds(lm_herb_aic, "lm_herb_aic.rds")

write_rds(lm_map_rsquared, "lm_map_rsquared.rds")
write_rds(lm_currentPPT_rsquared, "lm_currentPPT_rsquared.rds")
write_rds(lm_herb_rsquared, "lm_herb_rsquared.rds")

setwd('~/Desktop/Legacy Project')
lm_map_coef<-readRDS("lm_map_coef.rds")
lm_herb_coef<-readRDS("lm_herb_coef.rds")
lm_currentPPT_coef<-readRDS("lm_currentPPT_coef.rds")
```
...
map_ecoregion_spatiotemp_means <-map_coefficients %>%
  group_by(region) %>%
  summarise_at(vars(Satiotemporal),
                list(mean=mean,
                     se=std.error))
...
current_spatiotemp_means <-current_coefficients %>%
  group_by(region) %>%
  summarise_at(vars(Temporal.PPTdev),
                list(mean=mean,
                     se=std.error))
map_ecoregion_spatiotemp_means$top99<-map_ecoregion_spatiotemp_means$mean+(2.576*map_ecoregion_spatiotemp_means$se)
map_ecoregion_spatiotemp_means$bottom99<-map_ecoregion_spatiotemp_means$mean-(2.576*map_ecoregion_spatiotemp_means$se)
...
herb_ecoregion_spatiotemp_means <-herb_coefficients %>%
  group_by(region) %>%
  summarise_at(vars(herb.temporal),
                list(mean=mean,
                     se=std.error))
...
mapInteraction<-ggplot()+
  geom_density(data=map_coefficients, aes(x=Satiotemporal, fill=region,after_stat(scaled)),alpha=.3)+
  scale_fill_manual(values=c("indianred","skyblue","darkorchid","darkolivegreen3","gold"),
...
herbInteraction<-ggplot()+
  geom_density(data=herb_coefficients, aes(x=herb.temporal, fill=region,after_stat(scaled)),alpha=.3)+
  scale_fill_manual(values=c("indianred","skyblue","darkorchid","darkolivegreen3","gold"),
                       labels=c("Hot Deserts","Cold Deserts","California Annuals", "Northern Mixed Prairies","Shortgrass Steppe"))+
 xlab("Change in legacy effect per \n % change in herbaceous NPP")+
 ylab("Probability Density")+
 theme_classic() + 
  labs(fill = "Ecoregion")+
  theme(legend.key.size = unit(.75, 'cm'), #change legend key size
...
currentInteraction<-ggplot()+
  geom_density(data=current_coefficients, aes(x=Temporal.PPTdev, fill=region,after_stat(scaled)),alpha=.3)+
  scale_fill_manual(values=c("indianred","skyblue","darkorchid","darkolivegreen3","gold"),
                       labels=c("Hot Deserts","Cold Deserts","California Annuals", "Northern Mixed Prairies","Shortgrass Steppe"))+
 ...
 theme_classic() + 
  ...
  scale_y_continuous(expand = c(0,0),limits = c(0,1.02))+
    scale_x_continuous(expand = c(0,0),limits = c(-0.0007,.0025))+
  theme(axis.title.x =element_text( size=25),
        axis.title.y = element_text(size=25),
        axis.text.x = element_text(size=20),
        axis.text.y = element_text(size=20))+
...
```{r}
#create figure 4
jpeg("2 interaction figures.jpeg",
     width=18,
     height=10,
     units="in",
     res=300)
...
```{r}
#create figure 3b
jpeg("PPTdev.temporal.jpeg",
     width=10,
     height=8,
     units="in",
     res=300)
effect_byEco<-ggplot()+
  geom_density(data=current_coefficients, aes(x=Temporal.PPTdev, fill=region, after_stat(scaled)), alpha=.4)+
  scale_fill_manual(values=c("indianred","skyblue","darkorchid","darkolivegreen3","gold"),
                       labels=c("Hot Deserts","Cold deserts","California Annuals", "Northern Mixed \n Prairies","Shortgrass Steppe"))+
 ...
 theme_classic() + 
  ...
  scale_x_continuous(expand = c(0,0),limits = c(-.5,2.01))+
