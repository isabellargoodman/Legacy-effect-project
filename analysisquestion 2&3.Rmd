...
lm_map_dh[[i]]<-(df.dh.create(lm_map))
lm_currentPPT_dh[[i]]<-(df.dh.create(lm_currentPPT))
lm_herb_dh[[i]]<-(df.dh.create(lm_herb))

}
```

```{r}
#Calculate Durbins H-statistic 
library(ecm)
durbinH(lm_currentPPT, "lagNPPdev")

```

```{r}
with(test.strat, cor(NPPdev, lagNPPdev, use = "complete.obs"))
```
```{r}
test<-lm(NPPdev~PPTdev+lagNPPdev+PPTdev:lagNPPdev, data=test.strat.california_annuals)
durbinH(test,"lag")
```
